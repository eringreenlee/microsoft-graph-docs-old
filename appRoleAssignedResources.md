...
Retrieve the list of service principals that the user has an [appRoleAssignment](../resources/approleassignment.md) for. This operation also returns service principals assigned to groups that the user is a direct member of.
...
<!-- { "blockType": "ignored" } -->
```http
GET /users/{id | userPrincipalName}/appRoleAssignedResources
```
...
| ConsistencyLevel | eventual. This header and `$count` are required. It uses an index that may not be up-to-date with recent changes to the object. |
...
### Example 1: List service principals granted to a user
